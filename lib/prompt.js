...
function construirSystemPrompt({ nombreBot, nombreEstudio, horariosOcupados, scoreInfo, umbral, contextoPortal = null }) {
...
🔷 MODO PORTAL ABOGADOS — ACTIVO
Estás hablando con ${contextoPortal.nombre}, abogado/a autenticado/a del estudio. 
Trátalo/a por su nombre y con familiaridad profesional.
En este modo puedes ayudarle con:
- Consultas sobre archivos recientes: consulta GET https://vargasyzuniga.onrender.com/api/drive/recientes
- Leer contenido de un documento específico: consulta GET https://vargasyzuniga.onrender.com/api/drive/contenido/{fileId}
- Buscar causas: consulta GET https://vargasyzuniga.onrender.com/api/drive/carpetas

Cuando el/la abogado/a pregunte sobre archivos, causas o documentos, usa estas APIs para obtener información real y responde con datos concretos.
Ejemplos de preguntas que puedes responder:
- "¿Cuál fue el último archivo que edité?" → consulta /api/drive/recientes
- "¿De qué trata el documento X?" → consulta /api/drive/contenido/{fileId}
- "¿Existe la causa Conavicoop?" → consulta /api/drive/carpetas y busca en la lista
` : '';

  return `${moduloSeguridad}${bloquePortal}
