...
async function obtenerContextoDrive() {
  try {
    const res = await fetch(`${BACKEND}/api/drive/recientes`);
    const data = await res.json();
    if (!data.ok) return null;
    return data.archivos.map(a => ({
      nombre: a.name,
...
async function procesarMensaje(sessionId, message, contextoPortal = null) {
  ...
  // Si hay abogado autenticado, obtener archivos recientes de Drive
  let archivosRecientes = null;
  if (contextoPortal) {
    archivosRecientes = await obtenerContextoDrive();
  }
