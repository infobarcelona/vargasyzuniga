...
async function buscarArchivosDeCausa(carpetas, mensajeAbogado) {
  ...
  try {
    ...
    const carpetaEncontrada = carpetas.find(c => {
      const nombreLower = c.name.toLowerCase().normalize('NFD').replace(/[̀-ͯ]/g, '');
      // Verificar si alguna palabra del nombre de la carpeta aparece en el mensaje
      const palabras = nombreLower.split(/\s+/).filter(p => p.length > 3);
      return palabras.some(p => mensajeLower.includes(p));
    });

    if (!carpetaEncontrada) return null;
...
async function procesarMensaje(sessionId, message, contextoPortal = null) {
  ...
  if (contextoPortal) {
    ...
    console.log('[PORTAL] archivosDeCausa:', archivosDeCausa ? `encontrado: ${archivosDeCausa.carpetaNombre}` : 'null');
  }
