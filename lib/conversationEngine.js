...
async function procesarMensaje(sessionId, message, contextoPortal = null) {
  ...
  if (contextoPortal) {
    ...
    // Si encontró una causa nueva, guardarla en sesión
    if (archivosDeCausa) {
      session.ultimaCausaPortal = archivosDeCausa;
    }
    // Si no encontró causa en el mensaje pero el abogado se refiere a "esa causa" o "esa carpeta",
    // usar la última causa consultada en la sesión
    if (!archivosDeCausa && session.ultimaCausaPortal) {
      const palabrasReferencia = ['esa causa', 'esa carpeta', 'esa misma', 'dentro de esa', 'archivos de esa', 'qué hay', 'que hay', 'archivos hay', 'qué tiene', 'que tiene', 'contenido'];
      const mensajeLower = message.toLowerCase();
      if (palabrasReferencia.some(p => mensajeLower.includes(p))) {
        archivosDeCausa = session.ultimaCausaPortal;
      }
    }
