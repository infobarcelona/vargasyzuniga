...
async function procesarMensaje(sessionId, message, contextoPortal = null) {
  ...
  if (contextoPortal) {
    ...
    if (archivosDeCausa) {
      ...
      console.log('[PORTAL] quiereLeer:', quiereLeer, '| archivosDeCausa:', archivosDeCausa ? archivosDeCausa.carpetaNombre : 'null');
      if (quiereLeer) {
        // Buscar el archivo más relevante mencionado en el mensaje
        const mensajeNorm = mensajeLower.normalize('NFD').replace(/[̀-ͯ]/g, '');
        const archivoMencionado = archivosDeCausa.archivos.find(a => {
        ...
        console.log('[PORTAL] archivoMencionado:', archivoMencionado ? archivoMencionado.nombre : 'null');
