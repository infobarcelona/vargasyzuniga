...
function extraerHoraAudiencia(textoCompleto) {
  ...
  // Formatos: "10:00 horas", "a las 10:00", "10.00 horas", "10:00 hrs"
  const m1 = t.match(/(d{1,2})[:.h](d{2})s*(horas?|hrs?.?|am|pm)?/);
  if (m1) {
...
function extraerLugarAudiencia(textoCompleto) {
  ...
  // Buscar sala
  const sala = t.match(/salas*(n[°o]?s*)?(d+|[a-z]+)/i);
  // Buscar dirección
  const direccion = t.match(/(?:ubicado en|en calle|direcci[oó]n[:s]+|ens+)([A-Za-záéíóúÁÉÍÓÚñÑs/d#-]+?)(?:,|.|comuna|piso|
)/i);
  // Buscar comuna
  const comuna = t.match(/comunas+(?:des+)?([A-Za-záéíóúÁÉÍÓÚñÑs]+?)(?:,|.|$|
)/i);
  
  let lugar = '';
  if (sala) lugar += `Sala ${sala[2].toUpperCase()} — `;
