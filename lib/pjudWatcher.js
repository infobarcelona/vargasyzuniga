...
async function correrCicloWatcher() {
  ...
  for (const correo of correos) {
    ...
    if (correo.attachments.length === 0) {
      resumen.errores.push(`Correo "${correo.subject}" parecia relevante pero no trae Excel adjunto.`);
      await marcarProcesado(correo.messageId, { relevante: true, sinAdjunto: true, subject: correo.subject });
      continue;
    }
    ...
    if (!fechaAgenda) {
      resumen.errores.push(`No se pudo extraer la fecha del correo "${correo.subject}".`);
      await marcarProcesado(correo.messageId, { relevante: true, errorFecha: true, subject: correo.subject });
      continue;
    }
